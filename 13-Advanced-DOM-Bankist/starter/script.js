...
const revealSection = function (entries, observer) {
  ...
  observer.unobserve(entry.target); // "target", not "targets"
};
...
const imgObserver = new IntersectionObserver(loadImg, {
  root: null,
  threshold: 0,
  rootMargin: '+200px', // will lead to an early img load (b4 the user arrives at it)
});
...
const slider = function () {
  ...
  // initialization fn
  const init = function () {
    // places the slides side by side (0%, 100%, 200%......)
    goToSlide(0);
    // creates the dots
    createDots();
    // activates the initial dot
    activateDot(0);
  };
  ...
  /// Slider component Part II
  document.addEventListener('keydown', function (e) {
    console.log(e);
    if (e.key === 'ArrowLeft') prevSlide(); // works with if
    e.key === 'ArrowRight' && nextSlide(); // and short circuiting
  });
